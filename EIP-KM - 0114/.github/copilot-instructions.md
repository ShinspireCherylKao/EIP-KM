...
EIP-KM 是一個**前後台文件庫管理系統**，採用共享資料層設計：

```
前台 (前台/index.html)        後台管理 (後台管理/index.html)
    ↓                              ↓
...
各模組的功能規格書（SPEC.md）定義了業務邏輯、UI 行為與資料規則，開發時請遵循其中的定義：

- **前台**：`前台/SPEC.md`（文件庫管理 + KM 知識庫管理）
- **後台管理**：`後台管理/SPEC.md`
...
EIP-KM 系統需要在前台與後台管理兩個獨立頁面間實現資料同步與事件驅動的架構。前端開發需要理解共享資料層的設計、事件觸發機制與模組間通信的最佳實踐。
